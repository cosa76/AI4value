...
def analyze_business_context(df):
    insights = {"summary_statistics": df.describe(include='all').to_dict()}
    numeric = df.select_dtypes(include=['number']).columns
    for col in numeric:
        outliers = df[(df[col] > df[col].quantile(0.95)) | (df[col] < df[col].quantile(0.05))]
        insights[f"potential_outliers_in_{col}"] = len(outliers)
    return insights
...
if __name__ == "__main__":
    folder = "/Users/jndong/files_data_type8"
